...
<table id="users-table" class="table table-striped-column  table-sm table-striped table-fixed-first-column table-fixed-header" >
...
<tr id="{{$row}}">
   <td >{{$d->username}}</td>
    ...
    <i class="fa fa-edit text-primary fa-2x" ></i>
		...
		<a href="#" class="deleteDataBtnClass" deleteLabel="{{$d->username}}"  deleteId="{{$d->id}}" deleteRow="{{$row}}">
...
<div class="modal fade-scale" tabindex="-1" role="dialog" id="newDataModal" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="insert-user" method="post"  id="newDataForm">
			@csrf
      ...
      <div class="form-group">
          ...
          <?php $employees=DB::table('employees')->where('status',1)->get();?>
...
<div class="modal fade-scale" tabindex="-1" role="dialog" id="editDataModal" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="edit-user" method="post"   id="editDataForm">
			@csrf

      <input type="hidden" id="editId" name="id">
      <input type="hidden" id="editRow">
       <input type="hidden" name="employeeid" id="editemployee">
      <div class="form-group" >
				<label for="#">Employee</label>
		     <select   class="form-control" id="editemployee2" disabled>
          <?php $employees=DB::table('employees')->where('status',1)->get();?>
          @foreach($employees as $employee)
          <option value="{{$employee->id}}">{{$employee->name}}</option>
          @endforeach
...
$('#submitDataBtn').click(function(e) {
         ...
         data: $(form).serialize(),
          ...
          success: function(data) {
            ...
            if(data.status===201){
              ...
              toastr.info(data.info,'Info',{ timeOut : 15000 , 	progressBar: true})  
            }
              ...
              toastr.info('Un specified action occured','Unspeciffied Action',{ timeOut : 5000 , 	progressBar: true}); 
            }
          },
    ...
    $('#tbody').on('click', '.editDataBtnClass', function() {
      ...
      $('#editemployee').val($(this).attr('editemployee'));

      $('#editemployee2').val($(this).attr('editemployee'));
