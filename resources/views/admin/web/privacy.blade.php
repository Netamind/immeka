...
<a href="#" class="btn btn-primary" id="newDataBtn" style="float:right"><i class="fa fa-plus-circle" style="color:white"></i>New policy</a>
</h4>
<span style="font-size:18px">
Manage privacy policy displayed on the website
</span>
</div>
<div class="card-body">
...
<div class="tab-content" id="pills-tabContent">
    ...
    <div class="table-wrapper">
            <table id="privacy-table" class="table table-striped-column  table-sm table-striped table-fixed-first-column table-fixed-header" >
            <thead class="table-dark">
            <tr>
            <th class="table-dark">Policy</th>
            ...
            <tbody id="tbody">
            <?php
            $data = DB::table('privacy')->orderBy('id','asc')->get();
            ?>
            @foreach($data as $d)
            <?php $row = "row".$d->id;?>
            <tr id="{{$row}}">
                ...
                <i class="fa fa-edit text-primary fa-2x" ></i>
                    ...
                    <a href="#" class="deleteDataBtnClass" deleteLabel="{{$d->policy}}"  deleteId="{{$d->id}}" deleteRow="{{$row}}">
    ...
    <div class="row">
         ...
         <form action="#" method="post" id="dataTitleForm"> 
                @csrf
                <?php
                $privacyTitle=DB::table('privacytitle')->value('title');
                $privacyDescription=DB::table('privacytitle')->value('description');
                ?>

                 <div class="form-group">
                <label for="#" style="padding-bottom:5px">Title for privacy section</label>
                <input type="text" class="form-control" name="title" value="{{$privacyTitle}}" placeholder="Enter title for privacy section" autocomplete="off"> 
                </div>

                <div class="form-group">
                <label for="#" style="padding-bottom:5px">Description for privacy</label>
                <textarea name="description" class="form-control"  rows="4" >{{$privacyDescription}}</textarea>
                </div>


                <div class="form-group">
                <a href="#"  class="btn border-primary" style="margin-top:10px;float:right" id="updateDataTitleBtn">Update</a>
                </div>
                </form>  
    ...
    <div class="modal-content">
      ...
      <div class="row">
        ...
        <label for="#">Privacy  policy </label>
         <input type="text" name="policy" class="form-control" placeholder="Enter privacy policy" autocomplete="off" >
        </div>
...
<div class="modal fade-scale" tabindex="-1" role="dialog" id="editDataModal" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="col-md-12 form-group">
        <label for="#">Privacy ploicy</label>
         <input type="text" name="policy" class="form-control" id="editpolicy" autocomplete="off">
        </div>
...
<script>
 ...
 $(document).ready(function() {
  ...
  $(document).on("click", "#updateDataTitleBtn", function(e) {
  var self = $(this);
  $(this).prop("disabled", true);
  var form = document.getElementById("dataTitleForm");
  e.preventDefault(); 
      $.ajaxSetup({
          headers: {
      ...
      $.ajax({
        type:"post",
         url: '/admin-update-privacy-title',
         data: $(form).serialize(),
          timeout: 60000,
          beforeSend: function() {
...
$('#submitDataBtn').click(function(e) {
  ...
  $('#tbody').on('click', '.editDataBtnClass', function() {
    ...
    $('#editpolicy').val($(this).attr('editpolicy'));
